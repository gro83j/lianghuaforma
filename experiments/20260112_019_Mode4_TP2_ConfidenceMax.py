...
def run_round_019_mode4() -> int:
    ...
    out_dir = Path(env_out) if env_out else (base / "20260112")
    env_art = os.environ.get("ARTIFACTS_DIR")
    artifacts_dir = Path(env_art) if env_art else (out_dir / "019_artifacts")
    env_report = os.environ.get("REPORT_PATH")
    report_path = Path(env_report) if env_report else (out_dir / "019.txt")
    ...
    env_ref_cfg = os.environ.get("REF_CFG_PATH")
    ref_cfg_path = Path(env_ref_cfg) if env_ref_cfg else (out_dir / "016_artifacts" / "selected_config.json")
    ...
    thresholds_candidates: List[Path] = []
    env_thr = os.environ.get("THRESHOLDS_016_PATH")
    if env_thr:
        thresholds_candidates.append(Path(env_thr))
    ...
    if env_data:
        data_root_candidates.insert(0, Path(env_data))
